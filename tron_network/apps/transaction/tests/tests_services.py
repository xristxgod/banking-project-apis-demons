...
from apps.common.schemas import ResponseAllowance
from apps.transaction import schemas
from apps.transaction import services
from apps.transaction.services import TransactionType
from core.crypto.calculator import FEE_METHOD_TYPES
from core.crypto.utils import from_sun, to_sun
from apps.common.tests.factories import fake_private_key, fake_address, create_fake_contract
...
class TestTransfer:
    ...
    async def test_create(self, currency: str, commission: dict, mocker):
        ...
        mocker.patch(
            'apps.transaction.services.CreateTransfer.valid',
            side_effect=None,
        )
        ...
        mocker.patch(
            'apps.transaction.services.CreateTransfer._create',
            return_value={},
        )
...
class TestApprove:
    ...
    async def test_create(self, currency: str, commission: dict, mocker):
        ...
        mocker.patch(
            'apps.transaction.services.CreateApprove.valid',
            side_effect=None,
        )
        # There is no point in testing, the standard functionality is used, which has
        # already been tested in the `tronpy` library itself
        mocker.patch(
            'apps.transaction.services.CreateApprove._create',
            return_value={},
        )
...
class TestTransferFrom:
    ...
    async def test_valid(self, body: schemas.BodyCreateTransferFrom, fee: decimal.Decimal,
        ...
        mocker.patch(
            'apps.transaction.services.allowance',
            return_value=ResponseAllowance(
    ...
    async def test_create(self, currency: str, commission: dict, mocker):
        ...
        mocker.patch(
            'apps.transaction.services.CreateTransferFrom.valid',
            side_effect=None,
        )
        # There is no point in testing, the standard functionality is used, which has
        # already been tested in the `tronpy` library itself
        mocker.patch(
            'apps.transaction.services.CreateTransferFrom._create',
            return_value={},
        )
...
async def test_send_transaction(payload: dict, transaction_info: dict, mocker):
    ...
    mocker.patch(
        'apps.transaction.schemas.BodySendTransaction.create_transaction_obj',
        return_value=None,
    )
    # There is no point in testing, the standard functionality is used, which has
    # already been tested in the `tronpy` library itself
    mocker.patch(
        'apps.transaction.services.SendTransaction._send_transaction',
